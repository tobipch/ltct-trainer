...
for (const [key, caseData] of Object.entries(ltctMap)) {
  ...
  for (const [len, scrambles] of Object.entries(caseData.scrambles)) {
    for (let i = 0; i < Math.min(3, scrambles.length); i++) {
      ...
      // Check for U2' or R2' notation issues
      ...
      try {
        ...
        if (!edgesOk) {
          console.log(`EDGE FAIL: ${key} scramble[${len}][${i}]: edges=[${edges}]`);
          errors++;
        }
        if (!cornersOk) {
          console.log(`CORNER FAIL: ${key} scramble[${len}][${i}]`);
          errors++;
        }
      } catch(e) {
        console.log(`PARSE ERROR: ${key} scramble[${len}][${i}]: ${e.message}`);
        errors++;
      }
    }
  }
